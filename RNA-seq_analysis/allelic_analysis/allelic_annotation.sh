...
#annotate allelic genes
grep -Fwf Sample1_sig05_genes_final.cov genes.cov|cut -f 1 > Sample1_genes.cov

grep -Fwf Sample1_sig05_genes_final.cov genes.cov|cut -f 2 > Sample1_gene_names.cov

grep -Fwf Sample1_gene_names.cov Sample1_expression.txt|sort -k 1,1|cut -f 1|sort|uniq -u > Sample1_shared.cov

grep -Fwf Sample1_gene_names.cov final_genes.bed|sort -k 6,6|cut -f 6|sort|uniq -u > Sample1_genes_shared.cov

grep -Fwf Sample1_shared.cov Sample1_genes.shared.cov > Sample1_final.cov

grep -Fwf Sample1_final.cov Sample1_expression.txt|sort -k 1,1 > Sample1_shared.txt

grep -Fwf Sample1_final.cov final_genes.bed|sort -k 6,6 > Sample1_genes_shared.txt

paste Sample1_genes_shared.txt Sample1_shared.txt|cut -f 1,2,3,4,8,9 > Sample1_sig05_expression.bed
